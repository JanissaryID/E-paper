...
@Composable
fun ItemTextPotrait(sketchText: SketchText, itemText: ItemText, index: Int) {
...
//        rotation += rotationChange
    ...
    var stateOrientation by remember { mutableStateOf(itemText.indexOrientation) }

    Log.i("Sketch", "Orientation ${itemText.indexOrientation}")
        ...
        modifier = Modifier
            ...
            .pointerInput(Unit) {
                detectDragGestures { change, dragAmount ->
                    ...
                    if (offsetY < -((maxHeight / 2) - (size.height / 2).toFloat())) {
                        offsetY = -((maxHeight / 2) - (size.height / 2).toFloat())
                    }
                    if (offsetY > ((maxHeight / 2) - (size.height / 2).toFloat())) {
                        offsetY = ((maxHeight / 2) - (size.height / 2).toFloat())
                    }

                    if (offsetX < -((maxWidth / 2) - (size.width / 2).toFloat())) {
                        offsetX = -((maxWidth / 2) - (size.width / 2).toFloat())
                    }
                    if (offsetX > ((maxWidth / 2) - (size.width / 2).toFloat())) {
                        offsetX = ((maxWidth / 2) - (size.width / 2).toFloat())
                    }
                    Log.i("Sketch", "Potrait ${offsetX} -- $offsetY -- ${size}")
                }
            },
